...
function createProject() {
  ...
  for (let i = 0; i < projects.length; i++) {
    //adds all current projects in array to sidebar
    ...
    projLi.addEventListener("click", () => {
      //clicking on projLi will show all tasks that are related with that project
      document.getElementById("content").innerHTML = ""; //clears content

      for (let j = 0; j < taskList.length; j++) {
        //loops through taskList to find tasks that are related to the clicked projLi
    ...
    menu.addEventListener("click", () => {
      ...
      if (
        !adjLi.nextElementSibling ||
        adjLi.nextElementSibling.tagName !== "DIV"
      ) {
        //checks if projLi has div sibling already
        ...
        deleteBtn.addEventListener("click", () => {
          ...
          const updatedProjects = proj_list.filter(
            (proj) => proj !== projToDelete
          );
