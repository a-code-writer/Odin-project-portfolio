...
const createToDo = (newToDo, i) => {
  ...
  containerLeft.setAttribute(
    "style",
    "display: flex; gap: 5px; margin-left: 5px"
  );
  ...
  toDoCheck.setAttribute(
    "style",
    "max-height: 20px; max-width: 20px; align-self: center"
  );
  ...
  toDoName.addEventListener("click", () => {
    //on clicking the p element, the notes section will be appended to the bottom of the to-do
    if (
      !container.nextElementSibling ||
      (container.nextElementSibling.tagName !== "TEXTAREA" &&
        container.nextElementSibling.tagName !== "DIV" &&
        container.nextElementSibling.tagName !== "FORM")
    ) {
      //check if there is already a notes section, no duplicates
      document
        .getElementById("content")
        .insertBefore(toDoNotes, container.nextElementSibling);
    } else if (container.nextElementSibling.tagName === "TEXTAREA") {
      //if there is already a notes section, remove it
      toDoNotes.remove();
    }
  });
    ...
    if (
      ...
      toDoEdit.addEventListener("click", () => {
        if (
          ...
          //checks to see if the next element after container is a form. if not it appends the to-do update form to the bottom of the to-do content container div
          ...
          toDoFormPriority.innerHTML =
            "<option>Priority</option> <option>Not a Priority</option>";
          ...
          editForm.addEventListener("submit", (event) => {
            ...
            if (toDoFormPriority.value == "Not a Priority") {
              //checks if user selected Not priority or priority and uses if statment to set prioirty status to true or falsely
              newToDo.priority = "No";
            } else {
          ...
          document
            .getElementById("content")
            .insertBefore(editForm, container.nextElementSibling);
        }
      });
      ...
      toDoDelete.addEventListener("click", () => {
        ...
        const updatedLocalStorage = to_do_list.filter(
          (obj) => obj !== objToDelete
        );
      ...
      document
        .getElementById("content")
        .insertBefore(toDoBtnsDiv, container.nextElementSibling);
    }
  });
  ...
  container.setAttribute(
    "style",
    ` display: flex; justify-content: space-between; ${borderStyle} `
  );
